...
function displayUpcomingTrip(id, allTrips, allDestinations){
    let upcomingTrip = (0,_past_trips_js__WEBPACK_IMPORTED_MODULE_1__.viewUpcomingTrip)(id, allTrips)
    let locationOfTrip = allDestinations.find(location => {
        return location.id === upcomingTrip[0].destinationID
    }) 
    upcomingTripSection.innerText = `On ${(0,_dates_js__WEBPACK_IMPORTED_MODULE_5__.formatDate)(upcomingTrip[0].date)}, you will be leaving for ${locationOfTrip.destination} for ${upcomingTrip[0].duration} days!`
...
function displayPendingTrips(id, allTrips, allDestinations){
    ...
    if(pendingTrips.length === 0){
        ...
        pendingTrips.forEach(({travelers, destination, date, duration}) => {
            if(travelers === 1){
                postTripSection.innerHTML += `Currently waiting approval for a solo trip to ${destination} on ${(0,_dates_js__WEBPACK_IMPORTED_MODULE_5__.formatDate)(date)} for ${duration} day(s).<br><br>`
            } else if(travelers === 2) {
                postTripSection.innerHTML += `Currently waiting approval for a trip to ${destination} on ${(0,_dates_js__WEBPACK_IMPORTED_MODULE_5__.formatDate)(date)} with ${travelers - 1} other traveler for ${duration} day(s).<br><br>`
            } else {
            postTripSection.innerHTML += `Currently waiting approval for a trip to ${destination} on ${(0,_dates_js__WEBPACK_IMPORTED_MODULE_5__.formatDate)(date)} with ${travelers - 1} other travelers for ${duration} day(s).<br><br>`
            }    
        });
    };
